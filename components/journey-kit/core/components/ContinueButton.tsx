...
export function ContinueButton({ onPress, disabled = false, children }: ContinueButtonProps) {
  const handlePress = () => {
    if (!disabled) {
      onPress();
    }
    ...
    <MotiPressable
      ...
      <View className={`w-full rounded-full py-4 ${disabled ? 'bg-stone-100' : 'bg-stone-700'}`}>
        {children ? (
          ...
          <Text className={`text-center text-lg  ${disabled ? 'text-white' : 'text-stone-200'}`}>
            Continue
          </Text>
        )}
      </View>
